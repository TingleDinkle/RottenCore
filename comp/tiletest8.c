...
//#define FOR_ESP8266
//#define SUPERTINY
#define TINYWITHFLASH // To get 8x8
...
#define EVERY_OTHER_FRAME 0 // Was 1
#define SFILL 1
...
//			printf( "%3d ", closest );
	...
	else if( stage == 4 )
	{
		...
		{
			int tilew = 16;
			int tileh = (glyphct+tilew-1)/tilew;
			int tx, ty;
			printf( "Outputting %dx%d tiles\n", tilew, tileh );
			uint32_t * bitmap = malloc( tilew*tileh*10*10*4 );
			int stride = tilew*10;
			for(ty = 0; ty < tileh; ty++)
			for(tx = 0; tx < tilew; tx++)
			{
				int x, y;
				struct glyph * g = &gglyphs[tx+ty*tilew];
				for( y = 0; y < 10; y++ )
				for( x = 0; x < 10; x++ )
				{
					uint32_t color = 0xff000000;
					if( x < 9 && y < 9 && x > 0 && y > 0 )
						color |= (((g->dat.dat)>>(x-1+(y-1)*8))&1) ? 0xffffff : 0x000000;
					bitmap[(ty*10+y)*stride + (tx*10+x)] = color;
				}
			}
			stbi_write_png( "alltiles.png", tilew*10, tileh*10, 4, bitmap, stride*4);
			free( bitmap );
		}
